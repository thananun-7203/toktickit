...
function single(
  ...
  if (Array.isArray(raw)) {
    if (raw.length > 1) {
      errors[field] = `${field} must be provided at most once`;
      return undefined;
    }
...
function positiveInt(raw: unknown, field: string, errors: Record<string, string>): number | undefined {
  const value = single(raw, field, errors);
...
export function validateStaffQueueQuery(raw: Record<string, unknown>): {
  ...
  const searchRaw = single(raw.search, "search", errors);
  ...
  const itPriorityRaw = single(raw.itPriority, "itPriority", errors);
  ...
  const ownerRaw = single(raw.owner, "owner", errors);
  ...
  const sortRaw = single(raw.sort, "sort", errors);
  ...
  let page = 1;
  const pageRaw = single(raw.page, "page", errors);
  if (pageRaw) {
  ...
  let pageSize = 10;
  const pageSizeRaw = single(raw.pageSize, "pageSize", errors);
  if (pageSizeRaw) {
